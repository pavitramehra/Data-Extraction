...
(function dom__init() {
    ...
    pd.data             = {
        ...
        node         : {
            ...
            page        : (function dom__dataPage() {
                var divs = document.getElementsByTagName("comparebox");
                if (divs.length === 0) {
                    return null;
                }
                return divs[0];
    ...
    pd.event            = {
        ...
        reset        : function dom__event_reset() {
            ...
            localStorage.settings = "{\"feedback\":{\"newb\":" + pd.data.settings.feedback.newb + ",\"veteran\":" + pd.data.settings.feedback.veteran + "},\"codereport\":{},\"knownname\":" + pd.data.settings.knownname + "}";
    ...
    pd.event.grab       = function dom__event_grab(e, x) {
        var box        = (x.nodeName.toLowerCase() === "h3")
            ...
            filled     = ((pd.test.filled.stat === true) || (pd.test.filled.feed === true) || (box === pd.data.node.report.code.box && pd.test.filled.code === true)),
    ...
    load                = function dom__load() {
      ...
      /*      backspace       = function dom__load_backspace(event) {
        ...
        if (page === "webtool") {
            ...
            if (pd.data.settings.feedback.newb === false && pd.data.stat.usage > 2 && pd.data.stat.visit < 5 && pd.test.domain === true) {
            ...
            if (pd.data.settings.feedback.veteran === false && pd.data.stat.usage > 2500 && pd.test.domain === true) {
