...
function applyTooltip(id, alignToolTip) {
	if(id!=null && id != ""){
		if(!alignToolTip){alignToolTip = 'right';}
		$("#" + id).tooltip({
...
function populateServerSideError(id) {
	$('#'+id).parents('.form-group')
    .addClass('error');
	$('#'+id).parents('.form-group')
    .removeClass('success');
          
    if($('#'+id).parents('.outset-shadow-focus').length < 1 ) {
    	$('#'+id).parents('.form-group').addClass('outset-shadow-focus clearfix');
    }
}

function isValidDateee(txtDate,format)
{
  var currVal = txtDate;
  if(currVal == ''){
    return true;
  ...
  var date = Date.parseString(txtDate,format);
  if(date !=null){
    validFormat = true;
  }
...
function compareDate(elementId,minFieldValue,maxFieldValue,dateFormat) {
	var isValidDatee=isValidDateee($(elementId).val(),dateFormat);
...
function bindOnChangeEvent(id,pluginDateFormat,defDate){
	$("#"+id).bind("change",function(){
		...
		var datePattern = new RegExp("^[0-9]+[./-][0-9]+[./-][0-9]+$");//regex for date validation
		if(valueDatePicker){
			...
			if(finalDate != ""){$(this).val(finalDate);}
		} 
	});
	
	$("#"+id).bind("keyup",function(){ 
		if( typeof defDate != "undefined" && 
				(defDate != "" && ( (($.trim($(this).val()) == "") && ($.trim($(this).val()).length == 0))  || !isValidDateee($(this).val(),pluginDateFormat.replace(/([-./])M([-./])/i,'$1MMM$2')) ) ) ) {
...
function clearDate(id){	
	jQuery('#'+id).val('');
	var defDateToBeSet=$('#'+id).attr('defdate');
	$('#datepicker_'+id).datepicker({ autoclose: 1 });	
	$('#datepicker_'+id).data({date: defDateToBeSet}).datepicker('update');
}

//Function to show and hide result span in Select2 plugin.
function toggleShowHideSpan(id, hideSpan) {
	if(hideSpan == true) {
		$('ul#select2-'+id+'-results').parents('span.select2-container').addClass('select2-container--close').removeClass('select2-container--open');
	} else {
		$('ul#select2-'+id+'-results').parents('span.select2-container').addClass('select2-container--open').removeClass('select2-container--close');
	}
}